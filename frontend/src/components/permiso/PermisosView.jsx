...
const PermisosView = () => {
  const { permisos, loading, error, refreshPermisos } = usePermisos();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('');
  const [viewMode, setViewMode] = useState('cards'); // 'list' o 'cards'

  // Agrupar permisos por categoría
  const permisosByCategory = React.useMemo(() => {
    const grouped = {};
  ...
  const filteredPermisos = React.useMemo(() => {
    let filtered = { ...permisosByCategory };

    // Filtrar por categoría seleccionada
    if (selectedCategory) {
      filtered = { [selectedCategory]: permisosByCategory[selectedCategory] || [] };
    }
          ...
          <div key={category} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {permisos.map((permiso) => (
                <div key={permiso.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  {/* Header del card */}
                  <div className="flex items-start gap-3 mb-3">
                    <MdSecurity className="text-[#4EB9FA] flex-shrink-0 mt-1" size={20} />
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-800 text-sm truncate" title={permiso.nombre}>
                        {permiso.nombre}
                      </h4>
                    </div>
                  </div>
                  ...
                  {permiso.descripcion && (
                    <div className="mb-3">
                      <p className="text-gray-600 text-xs line-clamp-2" title={permiso.descripcion}>
                        {permiso.descripcion}
                      </p>
                    </div>
                  )}
                  ...
                  {(permiso.ruta || permiso.metodo) && (
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        {permiso.metodo && (
                          <span className={`inline-block px-2 py-1 text-xs font-semibold rounded border ${getMethodColor(permiso.metodo)}`}>
                            {permiso.metodo}
                          </span>
                        )}
                        {permiso.ruta && (
                          <div className="mt-1">
                            <code className="text-xs text-gray-700 bg-gray-50 px-2 py-1 rounded border font-mono block truncate" title={permiso.ruta}>
                              {permiso.ruta}
                            </code>
                          </div>
                        )}
