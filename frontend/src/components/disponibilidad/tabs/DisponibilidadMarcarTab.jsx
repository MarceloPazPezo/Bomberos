...
const DisponibilidadMarcarTab = () => {
  ...
  const handleAccesoRapido = async (preset) => {
    ...
    if (miDisponibilidad) {
      fireWarning('Disponibilidad Activa', 'Ya tienes una disponibilidad activa. Ciérrala antes de crear una nueva.');
      return;
    }
    ...
    try {
      ...
      switch (preset) {
        ...
        case '22h':
          ...
          break;
        case 'noche':
          let hasta6am = now.plus({ days: 1 }).set({ hour: 6, minute: 0, second: 0, millisecond: 0 });
  ...
  const handleCreateDisponibilidad = async () => {
    ...
    if (usarFechaTermino && fechaTermino) {
      ...
      if (terminoDateTime <= inicioDateTime) {
        fireError('Fecha de Término Inválida', 'La fecha de término debe ser posterior a la fecha de inicio');
        return;
      }
    }
  ...
  const handleCerrarDisponibilidad = async () => {
    ...
    try {
      ...
      fireError('Error en la Operación', 'Error al cerrar la disponibilidad. Inténtalo nuevamente.');
    } finally {
      setUpdatingMyStatus(false);
    }
  };
