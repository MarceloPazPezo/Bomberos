...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const generalLinks = [
    {
      to: "/partes-de-emergencias",
      label: "Partes de Emergencias",
      icon: <MdAssignment size={20} className="mr-2" />,
    },
    {
      to: "/calendario",
      label: "Calendario Operativo vista básica",
      icon: <MdEvent size={20} className="mr-2" />,
    },
  ];
  ...
  return (
    <>
      ...
      >
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm px-4 pt-4 pb-3 border-b border-[#dfe3ea] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-semibold tracking-wide text-[#2C3E50]">
                Panel de Control
              </span>
            </div>
          </div>
        ...
        <nav
          className={`flex-1 flex flex-col gap-1 px-4 pb-4 ${
            sidebarOpen
        ...
        >
          <p className="mt-3 mb-1 px-1 text-[11px] uppercase tracking-wider text-slate-500">
            Menu Principal
          </p>
          ...
          {hasPermiso("disponibilidad:obtener") && (
            ...
            >
              <span className="flex items-center">
                <MdHowToReg size={20} className="mr-2" />
                Disponibilidad
              </span>
            </NavLink>
          )}
          ...
          <div className="my-3 border-t border-[#dfe3ea]" />
          <p className="mb-1 px-1 text-[11px] uppercase tracking-wider text-slate-500">
            Administración
          </p>
          ...
          {hasPermiso("bombero:obtener") && (
            ...
            >
              <span className="flex items-center">
                <MdShield size={20} className="mr-2" />
                Inventario EPP
              </span>
            </NavLink>
          )}
          {(hasPermiso("puntoGeografico:obtener") || hasPermiso("puntoGeografico:admin")) && (
            <NavLink to="/mapa" onClick={() => setSidebarOpen(false)} className={getNavLinkClass}>
              <span className="flex items-center">
                <MdMap size={20} className="mr-2" />
                Mapa
              </span>
            </NavLink>
          )}
          ...
          {(hasPermiso("parte_emergencia:revisar") || hasPermiso("parte_emergencia:admin")) && (
            ...
            >
              <span className="flex items-center">
                <MdTask size={20} className="mr-2" />
                Revisar Partes de Emergencias
              </span>
            </NavLink>
          )}

          {(hasPermiso("evento:crear") || hasPermiso("evento:admin")) && (
            <NavLink
              to="/calendario-admin"
              onClick={() => setSidebarOpen(false)}
              className={getNavLinkClass}
            >
              <span className="flex items-center">
                ...
                Calendario Operativo vista admin
              </span>
            </NavLink>
          )}
          ...
          {(hasPermiso("bombero:admin") ||
            ...
            <NavLink to="/admin" onClick={() => setSidebarOpen(false)} className={getNavLinkClass}>
              <span className="flex items-center">
                <MdAdminPanelSettings size={20} className="mr-2" />
                Administración
              </span>
            </NavLink>
          )}
