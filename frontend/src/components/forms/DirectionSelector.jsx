...
const DirectionSelector = ({ 
  ...
  const { regiones: regionesRaw, comunas: comunasRaw, loadingComunas, loadingRegiones, fetchComunasByRegion, error } = useRegion();
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (newFormData.latitud && newFormData.longitud && 
  ...
  const regionName = formData.idRegion && regiones.length > 0
    ? regiones.find(r => r.id === parseInt(formData.idRegion))?.nombre || '' 
    : '';
  const comunaName = formData.idComuna && comunas.length > 0
  ...
  // Estado para controlar si el usuario ha seleccionado manualmente una ubicación en el mapa
  // o si hay coordenadas guardadas que deben mantenerse
  ...
  }, [comunaName, regionName, userSelectedLocation, formData.latitud, formData.longitud, isInitialLoad]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all ${
              errors.idRegion ? 'border-red-500 bg-red-50' : 'border-gray-300 bg-white'
            } ${disabled || loadingRegiones ? 'bg-gray-100 cursor-not-allowed' : 'hover:border-gray-400'}`}
          >
            <option value="">
            ...
            {regiones.length > 0 ? (
              ...
              !loadingRegiones && <option value="" disabled>No hay regiones disponibles</option>
            )}
          </select>
          {errors.idRegion && (
        ...
        <div>
          ...
          <select
            ...
            } ${disabled || !formData.idRegion ? 'bg-gray-100 cursor-not-allowed' : 'hover:border-gray-400'}`}
          >
            <option value="">
            ...
            <MapComponent
              ...
              initialLocation={(() => {
                ...
                if (mapLocation && 
                  ...
                  console.log('DirectionSelector - Pasando initialLocation a MapComponent:', mapLocation);
                  return mapLocation;
                }
