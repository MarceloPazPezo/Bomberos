...
const AdminBomberosTab = () => {
  ...
  const { openModal, closeModal, isModalOpen, getModalData } = useAdminModals();
  ...
  const handleDelete = useCallback(
    async (bomberos) => {
      ...
      const bomberosNames = bomberosArray
        .map((b) => {
    ...
    () => [
      {
        ...
        style: { width: "300px" },
        body: (rowData) => {
          const nombres = Array.isArray(rowData.nombres)
            ? rowData.nombres.join(" ")
            : rowData.nombres || "";
          const apellidos = Array.isArray(rowData.apellidos)
            ? rowData.apellidos.join(" ")
            : rowData.apellidos || "";
          return (
            <div className="flex flex-col py-2">
              <div className="font-semibold text-gray-900">{`${nombres} ${apellidos}`}</div>
              <div className="text-sm text-gray-600">RUN: {rowData.run}</div>
      ...
      },
      {
        ...
        body: (rowData) => {
          ...
          if (roles.length === 0) {
            return <span className="text-gray-500 italic">Sin roles</span>;
          }
          ...
          return (
            <div className="flex flex-wrap gap-1 items-center">
              ...
              <Tooltip content={`Todos los roles: ${allRolesText}`} place="top" variant="dark">
                <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-medium cursor-help hover:bg-gray-200 transition-colors">
                  +{remainingCount}
                </span>
              </Tooltip>
            </div>
        ...
        body: (rowData) => (
          <span
            className={`px-2 py-1 rounded-full text-xs font-medium ${
              rowData.activo ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
            }`}
          >
  ...
  const handleBomberoCreated = useCallback(
    async (newBombero) => {
      // El bombero ya fue creado en el popup, solo recargar la lista
      await fetchBomberos(true);
      closeModal("createBombero");
    },
    [fetchBomberos, closeModal]
  );

  const handleBomberoUpdated = useCallback(
    async (updatedBombero, run) => {
    ...
    <>
      <div className="bg-white/80 backdrop-blur-lg border border-[#4EB9FA]/20 shadow-md p-6 rounded-2xl">
        ...
        <PrimeTableAdvanced
          ...
          loading={false}
          onEdit={
            hasPermiso("bombero:actualizar") || hasPermiso("bombero:admin") ? handleEdit : undefined
          }
          onDelete={
            hasPermiso("bombero:eliminar") || hasPermiso("bombero:admin")
              ? handleDeleteSingle
              : undefined
          }
