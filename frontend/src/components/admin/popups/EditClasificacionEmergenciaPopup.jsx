...
import { updateClasificacionEmergencia } from '@services/clasificacionEmergencia.service';
import { toStartCase } from '@helpers/textFormatters.js';
import { toast } from 'react-toastify';

export default function EditClasificacionEmergenciaPopup({ show, setShow, data, onClasificacionEmergenciaUpdated, onUpdatingChange }) {
    const clasificacionData = data || {};
    ...
    // Cargar datos cuando cambia data
    useEffect(() => {
        if (show && clasificacionData) {
            setFormData({
                nombre: clasificacionData.nombre || ''
            });
            setErrors({});
        }
    }, [show, clasificacionData]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await updateClasificacionEmergencia(clasificacionData.id, { 
                nombre: formData.nombre.trim()
            });

            if (!result) {
                setErrors({ general: 'Error al actualizar la clasificación de emergencia' });
                setLoading(false);
                if (onUpdatingChange) onUpdatingChange(false);
                return;
            }

            toast.success(`Clasificación de emergencia "${toStartCase(result.nombre)}" actualizada exitosamente`);
            handleClose();
            
            if (onClasificacionEmergenciaUpdated) {
                onClasificacionEmergenciaUpdated(result);
            }
        } catch (error) {
    ...
    if (!show || !clasificacionData || !clasificacionData.id) return null;
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Editar Clasificación de Emergencia
                            </h2>
...
EditClasificacionEmergenciaPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    data: PropTypes.object,
    onClasificacionEmergenciaUpdated: PropTypes.func,
