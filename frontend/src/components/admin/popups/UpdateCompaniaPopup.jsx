...
export default function UpdateCompaniaPopup({ show, setShow, companiaData, onCompaniaUpdated, onUpdatingChange }) {
    ...
    useEffect(() => {
        if (show && companiaData) {
            const newFormData = {
                ...
                fechaFundacion: companiaData.fechaFundacion ? companiaData.fechaFundacion.split('T')[0] : '',
    ...
    const handleSubmit = async (formDataFromForm) => {
        // Usar los datos del formulario si están disponibles, sino usar el estado local
        const currentFormData = formDataFromForm || formData;
        ...
        try {
            ...
            const dataToSend = {
                ...
                ...(currentFormData.email && currentFormData.email.trim() !== '' && { email: currentFormData.email.trim() }),
                ...(currentFormData.telefono && currentFormData.telefono.trim() !== '' && { telefono: currentFormData.telefono.trim() }),
                ...(currentFormData.fechaFundacion && currentFormData.fechaFundacion.trim() !== '' && { fechaFundacion: currentFormData.fechaFundacion }),
                ...(currentFormData.descripcion && currentFormData.descripcion.trim() !== '' && { descripcion: currentFormData.descripcion.trim() }),
                ...(currentFormData.sitioWeb && currentFormData.sitioWeb.trim() !== '' && { sitioWeb: currentFormData.sitioWeb.trim() }),
                ...(currentFormData.idDireccion && { idDireccion: currentFormData.idDireccion })
            };

            const result = await updateCompania(companiaData.id, dataToSend, formData.logo, formData.banner);
            ...
            if (result.success) {
                ...
                } else if (result.message && result.message.includes('Ya existe otra compañía con ese correo electrónico')) {
                    setErrors({ email: 'Ya existe otra compañía con ese correo electrónico' });
                }
                // Los demás errores ya los maneja el hook con FireAlert
            }
        } catch (error) {
            console.error('Error updating compañía:', error);
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-sm">
                                Modifique la información de la compañía
                            </p>
                        </div>
                    </div>
                    <button
