...
import { createEstadoEpp } from '@services/estadoEpp.service';
import { toStartCase } from '@helpers/textFormatters.js';
import { toast } from 'react-toastify';

export default function CreateEstadoEppPopup({ show, setShow, onEstadoEppCreated, onCreatingChange }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Validar datos
            const estadoEppErrors = validateEstadoEppData(formData);
            ...
            // Crear estado EPP
            const result = await createEstadoEpp({ 
                nombre: formData.nombre.trim() 
            });

            if (!result) {
                setErrors({ general: 'Error al crear el estado de EPP' });
                setLoading(false);
                if (onCreatingChange) onCreatingChange(false);
                return;
            }

            toast.success(`Estado de EPP "${toStartCase(result.nombre)}" creado exitosamente`);
            handleClose();
            
            // Notificar al componente padre
            if (onEstadoEppCreated) {
                onEstadoEppCreated(result);
            }
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Error al crear el estado de EPP';
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Crear Nuevo Estado de EPP
                            </h2>
                            <p className="text-blue-100 text-sm">
                                Complete la información para registrar un nuevo estado de EPP
                            </p>
                        </div>
                    </div>
                    <button
...
CreateEstadoEppPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onEstadoEppCreated: PropTypes.func,
