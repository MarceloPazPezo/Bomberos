...
import { createSubtipoIncidente } from '@services/subtipoIncidente.service';
import { getClasificacionesEmergencia } from '@services/subtipoIncidente.service'; // Función pública para formularios
import { fetchClavesRadiales } from '@services/claveRadial.service';
import { toast } from 'react-toastify';

export default function CreateSubtipoIncidentePopup({ show, setShow, onSubtipoIncidenteCreated, onCreatingChange }) {
    ...
    const [formData, setFormData] = useState({
        ...
        descripcion: '',
        contieneFuego: false,
        contieneInmuebles: false,
        contieneVehiculos: false
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await createSubtipoIncidente({ 
            ...
            if (!result) {
                setErrors({ general: 'Error al crear el subtipo de incidente' });
                setLoading(false);
                if (onCreatingChange) onCreatingChange(false);
                return;
            }

            toast.success(`Subtipo de incidente "${result.claveRadial}" creado exitosamente`);
            handleClose();
            
            if (onSubtipoIncidenteCreated) {
                onSubtipoIncidenteCreated(result);
            }
        } catch (error) {
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Crear Nuevo Subtipo de Incidente
                            </h2>
                            <p className="text-blue-100 text-sm">
                                Complete la información para registrar un nuevo subtipo de incidente
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    ...
                                    <span>Crear Subtipo de Incidente</span>
                                </>
                            )}
                        </button>
                    </div>
...
CreateSubtipoIncidentePopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onSubtipoIncidenteCreated: PropTypes.func,
