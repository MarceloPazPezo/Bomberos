...
import { createClaveRadial } from '@services/claveRadial.service';
import { toStartCase } from '@helpers/textFormatters.js';
import { toast } from 'react-toastify';

export default function CreateClaveRadialPopup({ show, setShow, onClaveRadialCreated, onCreatingChange }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Validar datos
            const claveRadialErrors = validateClaveRadialData(formData);
            ...
            // Crear clave radial
            const result = await createClaveRadial({ 
                nombre: formData.nombre.trim() 
            });

            if (!result) {
                setErrors({ general: 'Error al crear la clave radial' });
                setLoading(false);
                if (onCreatingChange) onCreatingChange(false);
                return;
            }

            toast.success(`Clave radial "${toStartCase(result.nombre)}" creada exitosamente`);
            handleClose();
            
            // Notificar al componente padre
            if (onClaveRadialCreated) {
                onClaveRadialCreated(result);
            }
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Error al crear la clave radial';
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Crear Nueva Clave Radial
                            </h2>
                            <p className="text-blue-100 text-sm">
                                Complete la información para registrar una nueva clave radial
                            </p>
                        </div>
                    </div>
                    <button
...
CreateClaveRadialPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onClaveRadialCreated: PropTypes.func,
