...
export default function CreateRegionPopup({ show, setShow, onRegionCreated }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Validar datos
            const regionErrors = validateRegionData(formData);
            ...
            if (!result) {
                setErrors({ general: 'Error al crear la región' });
                setLoading(false);
                return;
            }
            ...
            if (onRegionCreated) {
                onRegionCreated(result);
            }
        } catch (error) {
            console.error('Error creating region:', error);
            const errorMessage = error.response?.data?.message || error.message || 'Error al crear la región';
