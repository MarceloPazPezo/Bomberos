...
import { updateTipoEpp } from '@services/tipoEpp.service';
import { toStartCase } from '@helpers/textFormatters.js';
import { toast } from 'react-toastify';

export default function EditTipoEppPopup({ show, setShow, data, onTipoEppUpdated, onUpdatingChange }) {
    const tipoEppData = data || {};
    ...
    // Validar datos del tipo EPP
    const validateTipoEppData = (data) => {
        const newErrors = {};
    ...
    const handleSubmit = async (formValues) => {
        ...
        try {
            // Validar datos
            const tipoEppErrors = validateTipoEppData(formValues);
            
            if (Object.keys(tipoEppErrors).length > 0) {
                setErrors(tipoEppErrors);
                setLoading(false);
            ...
            // Actualizar tipo EPP
            const result = await updateTipoEpp(tipoEppData.id, { 
                nombre: formValues.nombre.trim() 
            });

            if (!result) {
                setErrors({ general: 'Error al actualizar el tipo de EPP' });
                setLoading(false);
                if (onUpdatingChange) onUpdatingChange(false);
                return;
            }

            toast.success(`Tipo de EPP "${toStartCase(result.nombre)}" actualizado exitosamente`);
            handleClose();
            
            // Notificar al componente padre
            if (onTipoEppUpdated) {
                onTipoEppUpdated(result);
            }
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Error al actualizar el tipo de EPP';
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-sm">
                                Modifique la información del tipo de EPP
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 max-h-[65vh] bg-gray-50/50">
                    ...
                    <div className="space-y-6">
                        <Form
                            key={`edit-tipo-epp-${tipoEppData.id || 'new'}`}
                            ref={formRef}
                            title={null}
                            autoComplete="off"
                            size="w-full"
                            defaultValues={{
                                nombre: tipoEppData.nombre || ''
                            }}
                            fields={[
                                {
                                    label: "Nombre del Tipo de EPP",
                                    name: "nombre",
                                    placeholder: 'Ej: Casco, Chaqueta, Botas...',
                                    fieldType: 'input',
                                    type: "text",
                                    required: true,
                                    minLength: 2,
                                    ...
                                    defaultValue: tipoEppData.nombre || '',
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    ...
                                    <span>Actualizar Tipo de EPP</span>
                                </>
                            )}
                        </button>
                    </div>
...
EditTipoEppPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    data: PropTypes.object.isRequired,
    onTipoEppUpdated: PropTypes.func,
