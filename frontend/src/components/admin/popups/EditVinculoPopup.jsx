...
export default function EditVinculoPopup({ show, setShow, data, onVinculoUpdated, onUpdatingChange }) {
    const vinculoData = data || {};
    ...
    const handleSubmit = async (formValues) => {
        ...
        try {
            // Validar datos
            const vinculoErrors = validateVinculoData(formValues);
            ...
            // Actualizar vínculo
            const result = await updateVinculo(vinculoData.id, { 
                nombre: formValues.nombre.trim() 
            });

            if (!result) {
                setErrors({ general: 'Error al actualizar el vínculo' });
                setLoading(false);
                if (onUpdatingChange) onUpdatingChange(false);
                return;
            }

            toast.success(`Vínculo "${toStartCase(result.nombre)}" actualizado exitosamente`);
            handleClose();
            
            // Notificar al componente padre
            if (onVinculoUpdated) {
                onVinculoUpdated(result);
            }
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Error al actualizar el vínculo';
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-sm">
                                Modifique la información del vínculo
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 max-h-[65vh] bg-gray-50/50">
                    ...
                    <div className="space-y-6">
                        <Form
                            key={`edit-vinculo-${vinculoData.id || 'new'}`}
                            ref={formRef}
                            title={null}
                            autoComplete="off"
                            size="w-full"
                            defaultValues={{
                                nombre: vinculoData.nombre || ''
                            }}
                            fields={[
                                {
                                    ...
                                    defaultValue: vinculoData.nombre || '',
