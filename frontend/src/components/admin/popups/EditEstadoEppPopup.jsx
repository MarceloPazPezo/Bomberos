...
import { MdClose, MdCheckCircle, MdSave } from 'react-icons/md';
import PropTypes from 'prop-types';
import { updateEstadoEpp } from '@services/estadoEpp.service';
import { toStartCase } from '@helpers/textFormatters.js';
import { toast } from 'react-toastify';

export default function EditEstadoEppPopup({ show, setShow, data, onEstadoEppUpdated, onUpdatingChange }) {
    const estadoEppData = data || {};
    ...
    // Validar datos del estado EPP
    const validateEstadoEppData = (data) => {
        const newErrors = {};
    ...
    const handleSubmit = async (formValues) => {
        ...
        try {
            // Validar datos
            const estadoEppErrors = validateEstadoEppData(formValues);
            
            if (Object.keys(estadoEppErrors).length > 0) {
                setErrors(estadoEppErrors);
                setLoading(false);
                if (onUpdatingChange) onUpdatingChange(false);
                return;
            }

            // Actualizar estado EPP
            const result = await updateEstadoEpp(estadoEppData.id, { 
                nombre: formValues.nombre.trim() 
            });

            if (!result) {
                setErrors({ general: 'Error al actualizar el estado de EPP' });
                setLoading(false);
                if (onUpdatingChange) onUpdatingChange(false);
                return;
            }

            toast.success(`Estado de EPP "${toStartCase(result.nombre)}" actualizado exitosamente`);
            handleClose();
            
            // Notificar al componente padre
            if (onEstadoEppUpdated) {
                onEstadoEppUpdated(result);
            }
        } catch (error) {
            const errorMessage = error.response?.data?.message || error.message || 'Error al actualizar el estado de EPP';
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-[#4EB9FA] to-[#3A9BD9] rounded-t-2xl">
                    <div className="flex items-center space-x-3">
                        <div className="p-2 bg-white rounded-lg">
                            <MdCheckCircle className="w-6 h-6 text-[#3A9BD9]" />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-white">
                                Editar Estado de EPP
                            </h2>
                            <p className="text-blue-100 text-sm">
                                Modifique la información del estado de EPP
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 max-h-[65vh] bg-gray-50/50">
                    ...
                    <div className="space-y-6">
                        <Form
                            key={`edit-estado-epp-${estadoEppData.id || 'new'}`}
                            ref={formRef}
                            title={null}
                            autoComplete="off"
                            size="w-full"
                            defaultValues={{
                                nombre: estadoEppData.nombre || ''
                            }}
                            fields={[
                                {
                                    label: "Nombre del Estado de EPP",
                                    name: "nombre",
                                    placeholder: 'Ej: Disponible, En uso, Dañado, Mantenimiento...',
                                    fieldType: 'input',
                                    type: "text",
                                    required: true,
                                    minLength: 2,
                                    ...
                                    defaultValue: estadoEppData.nombre || '',
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    ...
                                    <span>Actualizar Estado de EPP</span>
                                </>
                            )}
                        </button>
                    </div>
...
EditEstadoEppPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    data: PropTypes.object.isRequired,
    onEstadoEppUpdated: PropTypes.func,
