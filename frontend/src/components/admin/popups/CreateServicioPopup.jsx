...
import { MdClose, MdLocalHospital, MdSave, MdInfo, MdError } from 'react-icons/md';
...
export default function CreateServicioPopup({ show, setShow, onServicioCreated, onCreatingChange }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.message && result.message.includes('Ya existe un servicio con ese nombre')) {
            ...
            if (error.response?.data?.message && error.response.data.message.includes('Ya existe un servicio con ese nombre')) {
                setErrors({ nombre: 'Ya existe un servicio con ese nombre' });
            }
            // Los demás errores ya los maneja el hook con FireAlert
        } finally {
                ...
                <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-[#4EB9FA] to-[#3A9BD9] rounded-t-2xl">
                    <div className="flex items-center space-x-3">
                        <div className="p-2 bg-white rounded-lg">
                            <MdLocalHospital className="w-6 h-6 text-[#3A9BD9]" />
                        </div>
                        <div>
                            ...
                            <p className="text-blue-100 text-sm">
                                Agregue un nuevo servicio al sistema
                            </p>
                        </div>
                    </div>
                    <button
                                ...
                                fields={[
                                    {
                                        ...
                                        fieldType: 'input',
                                        type: "text",
                                        placeholder: "Ej: SAMU, Carabineros, Bomberos, Policía",
                                        required: true,
                                        minLength: 2,
                                        maxLength: 100,
...
CreateServicioPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onServicioCreated: PropTypes.func,
