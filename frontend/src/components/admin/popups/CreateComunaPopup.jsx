...
import { createComuna } from '@services/region.service';
import { fetchRegiones } from '@services/region.service';
import { toast } from 'react-toastify';

export default function CreateComunaPopup({ show, setShow, onComunaCreated }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Validar datos
            const comunaErrors = validateComunaData(formData);
            ...
            // Crear comuna
            const result = await createComuna({ 
                nombre: formData.nombre.trim(),
                idRegion: parseInt(formData.idRegion)
            });

            if (!result) {
                setErrors({ general: 'Error al crear la comuna' });
                setLoading(false);
                return;
            }
            ...
            if (onComunaCreated) {
                onComunaCreated(result);
            }
        } catch (error) {
            console.error('Error creating comuna:', error);
            const errorMessage = error.response?.data?.message || error.message || 'Error al crear la comuna';
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-blue-100 text-sm">
                                Complete la información para registrar una nueva comuna y asignarla a una región
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex justify-end px-6 py-4 bg-white border-t border-gray-200 rounded-b-2xl">
                    <div className="flex space-x-3">
                        ...
                        <button
                            ...
                            disabled={loading || !formData.nombre.trim() || !formData.idRegion}
                        >
                            {loading ? (
...
CreateComunaPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onComunaCreated: PropTypes.func
};
