...
export default function CreateTipoEventoPopup({ show, setShow, onTipoEventoCreated }) {
    ...
    const handleSubmit = async (createdTipoEventoData) => {
        if (createdTipoEventoData) {
            setLoading(true);
            try {
                // Validar formulario completo
                const validationErrors = validateForm(createdTipoEventoData);
                ...
                if (onTipoEventoCreated) {
                    const result = await onTipoEventoCreated(createdTipoEventoData);
                    if (result.success) {
                        setShow(false);
                        setErrors({});
                    ...
                    <div className="relative w-full max-w-xs sm:max-w-2xl h-auto p-0 animate-fade-in flex flex-col rounded-2xl bg-white shadow-2xl border border-gray-200 max-h-[90vh]">
                        ...
                        <div className="flex items-center px-4 sm:px-6 py-4 bg-gradient-to-r from-[#4EB9FA] to-[#3A9BD9] rounded-t-2xl">
                            <div className="flex items-center space-x-3 flex-1">
                                ...
                                <h2 className="text-xl font-bold text-white">Crear Tipo de Evento</h2>
                            </div>
                            <button
...
CreateTipoEventoPopup.propTypes = {
    show: PropTypes.bool.isRequired,
    setShow: PropTypes.func.isRequired,
    onTipoEventoCreated: PropTypes.func.isRequired,
};
