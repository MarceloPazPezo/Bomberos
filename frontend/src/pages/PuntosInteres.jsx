    ...
    const initMap = setTimeout(() => {
      ...
      try {
        ...
        map.current = new maplibregl.Map({
          ...
          center: [-72.4, -37.0333], // Cabrero, Región del Bío-Bío
          zoom: 13,
  ...
  useEffect(() => {
    ...
    if (coordsDibujo.length > 0) {
      ...
      map.current.addLayer({
        id: "juris-draw-points",
        type: "circle",
        source: "juris-draw-points-src",
        paint: {
          "circle-radius": 4,
          "circle-color": "#0ea5e9",
    ...
    if (coordsDibujo.length >= 2) {
      const line = {
        type: "Feature",
        geometry: { type: "LineString", coordinates: coordsDibujo },
        properties: {},
      };
    ...
    if (coordsDibujo.length >= 3) {
      ...
      map.current.addLayer({
        id: "juris-draw-fill",
        type: "fill",
        source: "juris-draw-poly-src",
        paint: {
          "fill-color": jurisForm.color || "#22c55e",
          "fill-opacity": 0.15,
        },
      });
      map.current.addLayer({
        ...
        source: "juris-draw-poly-src",
        paint: {
          "line-color": jurisForm.color || "#16a34a",
          "line-width": 2,
        },
      });
    }
                ...
                `
                                <div class="p-5 min-w-[450px] max-w-[600px]">
                                    ...
                                    <div class="flex items-start gap-4 mb-4 pb-4 border-b border-gray-200">
                                        <!-- Icono grande -->
                                        <div class="flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center text-white shadow-lg" style="background: linear-gradient(135deg, ${
                                          j.color || "#16a34a"
                                        } 0%, ${j.color ? j.color + "dd" : "#16a34a"} 100%)">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h3 class="text-xl font-bold text-gray-900 mb-1 leading-tight">${toStartCase(
                                              j.nombre || "Jurisdicción"
                                            )}</h3>
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold" style="background-color: ${
                                                  j.color || "#16a34a"
                                                }20; color: ${j.color || "#16a34a"}">
                                                    Área de cobertura
                                                </span>
                                                ${
                                    ...
                                    ${
                                      j.descripcion
                                        ? `
                                        <div class="mb-4">
                                            <p class="text-sm text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">${toStartCase(
                                              j.descripcion
                                            )}</p>
                                        </div>
                                    `
                                    ...
                                    <div class="grid grid-cols-2 gap-3 mb-4">
                                        ...
                                        <div class="bg-white border border-gray-200 rounded-lg p-3">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <span class="inline-block w-4 h-4 rounded-full border-2 border-gray-300 shadow-sm" style="background-color:${
                                                  j.color || "#16a34a"
                                                }"></span>
                                                <span class="text-sm text-gray-800 font-mono text-xs">${
                                                  j.color || "#16a34a"
                                                }</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="bg-white border border-gray-200 rounded-lg p-3">
                                            ...
                                            <div class="flex items-center gap-1.5 text-sm text-gray-800">
                                                ...
                                                <span class="font-medium font-mono text-xs">${
                                                  j.creadoEl ? formatDate(j.creadoEl) : "N/D"
                                                }</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex gap-3 pt-3 border-t border-gray-200">
                                        <button 
                                            onclick="window.editarJurisdiccion && window.editarJurisdiccion(${
                                              j.id
                                            })"
        ...
        maxWidth: "550px",
                    ...
                    <div class="p-4 min-w-[400px] max-w-[550px]">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-white shadow-sm" style="background-color: ${
                              punto.tipoPunto?.color || "#FF0000"
                            }">
                                ${
                                  punto.tipoPunto?.icono
                                    ? getIconoSvg(punto.tipoPunto.icono)
                            ...
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-gray-900 leading-tight">${toStartCase(
                                  punto.nombre
                                )}</h3>
                                <p class="text-sm text-gray-500 font-medium">${toStartCase(
                                  punto.tipoPunto?.nombre || "Sin tipo"
                                )}</p>
                            </div>
                        </div>
                        ...
                        ${
                          punto.descripcion
                            ? `
                            <div class="mb-3">
                                <p class="text-sm text-gray-700 leading-relaxed">${toStartCase(
                                  punto.descripcion
                                )}</p>
                            </div>
                        `
                        ...
                        <div class="space-y-2 mb-3">
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                <span class="text-gray-800">${toStartCase(
                                  punto.compania?.nombre || "N/A"
                                )}</span>
                            </div>
                            <div class="flex items-center gap-2 text-sm text-gray-600">
                                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex gap-2 pt-2 border-t border-gray-100">
                            <button onclick="window.editarPunto(${
                              punto.id
                            ...
                            <button onclick="window.eliminarPunto(${
                              punto.id
                            })" class="flex-1 bg-red-600 hover:bg-red-700 text-white text-sm font-medium py-2 px-3 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  useEffect(() => {
        ...
        setTimeout(() => {
          if (maplibregl && map.current) {
            ...
            tempMarkerRef.current.on("dragend", () => {
              ...
              toast.success(
                `Ubicación actualizada: ${lngLat.lat.toFixed(6)}, ${lngLat.lng.toFixed(6)}`,
                {
                  duration: 2000,
    ...
    window.editarJurisdiccion = (id) => {
      ...
      if (jurisdiccion) {
        ...
        if (
          jurisdiccion.coordenadas &&
          Array.isArray(jurisdiccion.coordenadas) &&
          jurisdiccion.coordenadas.length > 0
        ) {
        ...
        <div className="flex-1 flex overflow-hidden min-h-0">
          ...
          <div className="flex-1 relative min-w-0">
            ...
            <button
              ...
              aria-label={
                mostrarJurisdicciones ? "Ocultar jurisdicciones" : "Mostrar jurisdicciones"
              }
            >
              {mostrarJurisdicciones ? (
