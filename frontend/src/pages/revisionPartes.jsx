...
function TablaRevision({ rows, onOpen, onChangeEstado }) {
  ...
  const tableRows = useMemo(() => {
    ...
    return (rows || [])
      .filter((r) => estadosPermitidos.has(String(r?.estado || "").toUpperCase()))
      .map((r) => ({
        ...r,
  ...
  const estadoBody = (row) => {
    ...
    const sev =
      e === "APROBADO" ? "success" : e === "ENVIADO" ? "info" : e === "CORREGIR" ? "danger" : null;
    return <Tag value={row.estado} severity={sev} />;
  };

  const descripcionBody = (row) => <span title={row.descripcion}>{trim(row.descripcion, 60)}</span>;
  ...
  return (
    ...
    >
      <Column
        ...
        sortable
        body={(row) => row.fecha}
        headerClassName="!py-2 !px-2 text-xs"
        bodyClassName="!py-1 !px-2 text-xs"
        style={{ minWidth: "8rem" }}
      />
      <Column
        field="estado"
        ...
        body={estadoBody}
        filter
        filterElement={estadoFilterElement}
      ...
      <Column
        ...
        sortable
        filter
        body={(row) => row.claveRadial}
        filterElement={claveFilterElement}
        showFilterMatchModes={false}
        headerClassName="!py-2 !px-2 text-xs"
        bodyClassName="!py-1 !px-2 text-xs"
        style={{ minWidth: "8rem" }}
...
export default function RevisionPartes() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const filtrados = (Array.isArray(data) ? data : []).filter((r) =>
        estadosPermitidos.includes(String(r?.estado || "").toUpperCase())
      );
      setRows(filtrados);
    } finally {
      setLoading(false);
  ...
  const onChangeEstado = (incidente) => {
    // Mostrar diálogo para elegir aprobar o rechazar
    fireAlert.fireInfo(
      "Cambiar estado",
  ...
  const confirmChange = async (comentario) => {
    if (!incidenteSeleccionado?.id) return;
    ...
    try {
      ...
      await cambiarEstadoIncidente(incidenteSeleccionado.id, {
        estado: nuevoEstado,
        idBombero: 1,
        comentario,
      });
  ...
  return (
    <div className="min-h-[80vh]">
        ...
        <div className="bg-white/80 backdrop-blur-lg border border-[#4EB9FA]/20 shadow-md rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-[#2C3E50]">Revisar Partes de Emergencias</h1>
              </div>
              <Tooltip
      ...
      <ModalRevisionParte
        ...
        tipo={tipoModal}
        incidenteId={incidenteSeleccionado?.id}
      />
    </div>
  );
}
