...
import {
  ...
  createJurisdiccion,
  updateJurisdiccion,
} from "@services/jurisdiccion.service";
import { getCompaniasConCoordenadas } from "@services/compania.service";
...
const Mapa = () => {
      ...
      return (
        <span
          ...
          dangerouslySetInnerHTML={{
            __html: getIconoSvg("CustomHydrant").replace("24 24", "20 20"),
          }}
        />
      );
    }
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const [puntosData, tiposData, jurisdiccionesData, companiasData] = await Promise.all([
        getPuntosGeograficos(),
        getTiposPunto(),
        getJurisdicciones(),
      ...
      setCompanias(Array.isArray(companiasData?.data) ? companiasData.data : []);
  ...
  const onLoad = useCallback(function callback(map) {
    ...
    map.setCenter({ lat: -37.0333, lng: -72.4 });
    map.setZoom(13);
    setMap(map);
    mapRef.current = map;
  }, []);
  ...
  const handleEditJurisdiccion = (id) => {
    ...
    if (jurisdiccion) {
      ...
      if (jurisdiccion.coordenadas && Array.isArray(jurisdiccion.coordenadas)) {
        const coords = jurisdiccion.coordenadas.map((c) => ({
            ...
            <div className="flex items-center gap-3 bg-gray-50 px-4 py-2 rounded-lg border border-gray-200">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFiltrosCategorias((prev) => ({ ...prev, PUNTO_INTERES: e.target.checked }))
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFiltrosCategorias((prev) => ({
                      ...prev,
                      UBICACION_BOMBERO: e.target.checked,
                    }))
                  }
              ...
              {/* <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={filtrosCategorias.COMPANIA}
                  onChange={(e) =>
                    setFiltrosCategorias((prev) => ({ ...prev, COMPANIA: e.target.checked }))
                  }
                  className="w-4 h-4 text-[#4EB9FA] rounded focus:ring-2 focus:ring-[#4EB9FA]"
                />
                <span className="text-sm text-gray-700">Compañías</span>
              </label> */}
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {ultimaActualizacion && (
                <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
                  <span className="text-sm text-green-700 font-medium">Última actualización:</span>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex-1 relative min-w-0">
          ...
          >
            ...
            {/* Marcadores de Compañías */}
            {filtrosCategorias.COMPANIA &&
              companias.map((compania) => {
                if (!compania.latitud || !compania.longitud) {
                  return null;
                }

                try {
                  const position = {
                    lat: parseFloat(compania.latitud),
                    lng: parseFloat(compania.longitud),
                  };

                  if (isNaN(position.lat) || isNaN(position.lng)) {
                    console.error("Coordenadas inválidas para compañía:", compania);
                    return null;
                  }

                  const icon = {
                    path: "M12,2C8.13,2,5,5.13,5,9c0,5.25,7,13,7,13s7-7.75,7-13C19,5.13,15.87,2,12,2z M12,11c-1.1,0-2-0.9-2-2s0.9-2,2-2s2,0.9,2,2S13.1,11,12,11z",
                    fillColor: "#4EB9FA",
                    fillOpacity: 1,
                    strokeWeight: 2,
                    strokeColor: "#FFFFFF",
                  ...
                  return (
                    <Marker
                      key={`compania-${compania.id}`}
                      position={position}
                      onClick={() =>
                        setSelectedMarker({
                          ...compania,
                          isCompania: true,
                          coordenadas: { lat: compania.latitud, lng: compania.longitud },
                        })
                      }
                      icon={icon}
                    />
                  ...
                  console.error("Error renderizando marcador de compañía:", err, compania);
                  return null;
                }
              })}
            ...
            {mostrarJurisdicciones &&
              jurisdicciones.map((jurisdiccion) => (
                <Polygon
                  key={jurisdiccion.id}
                  paths={jurisdiccion.coordenadas.map((c) => ({
                    lat: parseFloat(c.lat),
                    lng: parseFloat(c.lng),
                  }))}
                  options={{
            ...
            {selectedMarker && (
              <InfoWindow
                position={{
                  lat: parseFloat(selectedMarker.coordenadas.lat),
                  lng: parseFloat(selectedMarker.coordenadas.lng),
                }}
                onCloseClick={() => setSelectedMarker(null)}
              >
                <div className="p-2 min-w-[300px]">
                  {selectedMarker.isCompania ? (
                    /* Vista para Compañías */
                    <>
                      <div className="flex items-start gap-4 mb-4 pb-4 border-b border-gray-200">
                        <div className="flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#4EB9FA] to-[#3A9DD9] text-white shadow-lg">
                          <MdBusiness className="w-8 h-8" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xl font-bold text-gray-900 mb-1 leading-tight">
                            {toStartCase(selectedMarker.nombre)}
                          </h3>
                          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-[#4EB9FA]/20 text-[#4EB9FA]">
                      ...
                      <div className="space-y-3">
                        {selectedMarker.email && (
                          <div className="flex items-start gap-2">
                            ...
                            <span className="text-sm text-gray-800">{selectedMarker.email}</span>
                          </div>
                        )}
                        {selectedMarker.telefono && (
                          <div className="flex items-start gap-2">
                            ...
                            <span className="text-sm text-gray-800">{selectedMarker.telefono}</span>
                          </div>
                        )}
                        {selectedMarker.calle && selectedMarker.numero && (
                          <div className="flex items-start gap-2">
                            <span className="text-xs font-medium text-gray-500 min-w-[60px]">
                              Dirección:
                            </span>
                            <span className="text-sm text-gray-800">
                              {selectedMarker.calle} {selectedMarker.numero}
                              {selectedMarker.depto ? `, ${selectedMarker.depto}` : ""}
                            </span>
                          </div>
                        )}
                        {selectedMarker.comuna && (
                          <div className="flex items-start gap-2">
                            <span className="text-xs font-medium text-gray-500 min-w-[60px]">
                              Comuna:
                            </span>
                        ...
                        <div
                          ...
                          style={{
                            background: `linear-gradient(135deg, ${
                              selectedMarker.tipoPunto?.color || "#FF0000"
                            } 0%, ${
                              selectedMarker.tipoPunto?.color
                                ? selectedMarker.tipoPunto.color + "dd"
                                : "#CC0000"
                            } 100%)`,
                          }}
                        >
                          {selectedMarker.tipoPunto?.icono ? (
                            <div
                              dangerouslySetInnerHTML={{
                                __html: getIconoSvg(selectedMarker.tipoPunto.icono)
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              ...
                              style={{
                                ...
                                color: selectedMarker.tipoPunto?.color || "#FF0000",
                              }}
                            >
                      ...
                      {selectedMarker.descripcion && (
                        <div className="mb-4">
                          <p className="text-sm text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">
                            {toStartCase(selectedMarker.descripcion)}
                          </p>
                        </div>
                      )}
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg p-3">
                          ...
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold text-white shadow-sm ${
                              selectedMarker.estado === "BUENO"
                                ? "bg-green-500"
                                : selectedMarker.estado === "REGULAR"
                                ? "bg-yellow-500"
                                : selectedMarker.estado === "MALO"
                                ? "bg-red-500"
                                : "bg-gray-500"
                            }`}
                          >
                ...
                <div className="p-2 min-w-[300px]">
                  <div className="flex items-start gap-4 mb-4 pb-4 border-b border-gray-200">
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${
                          selectedJurisdiccion.color || "#16a34a"
                        } 0%, ${
                          selectedJurisdiccion.color ? selectedJurisdiccion.color + "dd" : "#16a34a"
                        } 100%)`,
                      }}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-xl font-bold text-gray-900 mb-1 leading-tight">
                        {toStartCase(selectedJurisdiccion.nombre || "Jurisdicción")}
                      </h3>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span
                          className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold"
                          style={{
                            backgroundColor: `${selectedJurisdiccion.color || "#16a34a"}20`,
                            color: selectedJurisdiccion.color || "#16a34a",
                          }}
                        >
                          Área de cobertura
                        </span>
                  ...
                  {selectedJurisdiccion.descripcion && (
                    <div className="mb-4">
                      <p className="text-sm text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100">
                        {toStartCase(selectedJurisdiccion.descripcion)}
                      </p>
                    </div>
                  )}
          ...
          <button
            type="button"
            onClick={() => {
              setMostrarJurisdicciones((v) => !v);
            }}
            title={mostrarJurisdicciones ? "Ocultar jurisdicciones" : "Mostrar jurisdicciones"}
            className={`absolute right-4 top-1/2 transform -translate-y-1/2 z-10 inline-flex items-center justify-center w-10 h-10 rounded-full shadow bg-white border ${
              mostrarJurisdicciones
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm font-medium text-blue-800 flex items-center gap-2 mb-1">
                    ...
                    {editingJurisId
                      ? "Modifica el polígono haciendo clic en el mapa"
                      : "Haz clic en el mapa para agregar vértices"}
                  </p>
                  <p className="text-xs text-blue-700">
                    Puntos agregados: <span className="font-semibold">{coordsDibujo.length}</span>{" "}
                    {coordsDibujo.length < 3 && "(mínimo 3)"}
                  </p>
                </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.error(
                          editingJurisId
                            ? "Error al actualizar la jurisdicción"
                            : "Error al crear la jurisdicción"
                        );
                      } finally {
                        setLoading(false);
                      }
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(option) =>
                      setFormData({ ...formData, idTipoPunto: option?.value || "" })
                    }
                    ...
                    <div className="flex items-center gap-2 mt-2 text-sm text-gray-600">
                      {renderIcon(
                        tiposPunto.find((t) => t.id === parseInt(formData.idTipoPunto))?.icono,
                        {
                          className: "w-5 h-5",
                          style: {
                            color: tiposPunto.find((t) => t.id === parseInt(formData.idTipoPunto))
                              ?.color,
                          },
                        }
                      )}
                      <span>
                        {tiposPunto.find((t) => t.id === parseInt(formData.idTipoPunto))?.nombre}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(option) =>
                      setFormData({ ...formData, estado: option?.value || "BUENO" })
                    }
                  ...
                  />
                  <div className="flex items-center gap-2 mt-2 text-sm text-gray-600">
                    {formData.estado === "BUENO" && (
                      <>
                        <MdCheckCircle className="text-green-600" /> Estado Bueno
                      </>
                    )}
                    {formData.estado === "REGULAR" && (
                      <>
                        <MdWarning className="text-yellow-600" /> Estado Regular
                      </>
                    )}
                    {formData.estado === "MALO" && (
                      <>
                        <MdCancel className="text-red-600" /> Estado Malo
                      </>
                    )}
                    {formData.estado === "FUERA_DE_SERVICIO" && (
                      <>
                        <MdBuild className="text-gray-600" /> Fuera de Servicio
                      </>
                    )}
                  </div>
                </div>
                ...
                {user?.companiaId && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    ...
                    <p className="text-sm text-blue-700">ID: {user.companiaId}</p>
