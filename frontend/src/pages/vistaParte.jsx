...
import {
  generarReporteParteEmergenciaPdf,
    ...
    (async () => {
      try {
        ...
        const [
          ...
          p.comunaId
            ? regionService.getComunaById(p.comunaId).catch(() => null)
          ...
          if (Array.isArray(bomberosData)) {
            bomberosData.forEach((b) => {
              const nombreCompleto =
                [b.nombres, b.apellidos].filter(Boolean).join(" ").trim() || null;
        ...
        const comunaObj = comunaDetalleData || findById(comunasArr, p.comunaId) || null;
        const bomberoById = (id) => {
          const b = findById(bomberosArr, id);
          if (!b) return id ? { id, nombreCompleto: `Bombero #${id}` } : null;
          const nombreCompleto =
            [b.nombres, b.apellidos].filter(Boolean).join(" ").trim() || `Bombero #${b.id}`;
        ...
        const parteDet = {
          ...
          incendio: {
            tipo: tipoDanoObj
              ...
              : p.tipoIncendioId
              ? { id: p.tipoIncendioId, nombre: null }
              : null,
            fase: faseObj
              ? { id: faseObj.id, nombre: faseObj.nombre }
              : p.faseId
              ? { id: p.faseId, nombre: null }
              : null,
          },
          ...
          accidentados: Array.isArray(p.accidentados)
            ? p.accidentados.map((a) => ({
                bombero:
                  bomberoMap[a.bomberoId] ||
                  (a.bomberoId ? { id: a.bomberoId, nombreCompleto: null } : null),
                compania:
                  companiaMap[a.companiaId] ||
                  (a.companiaId ? { id: a.companiaId, nombre: null } : null),
                lesiones: a.lesiones || "",
    ...
    <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
      ...
      {parte?.clasificacion && (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-sky-200 bg-sky-50 text-sky-800">
          ...
          {parte.clasificacion.nombre || `Clasificación #${parte.clasificacion.id}`}
        </span>
      )}
      {parte?.incendio?.tipo && (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-orange-200 bg-orange-50 text-orange-800">
          ...
          {parte.incendio.tipo.nombre || `Tipo Incendio #${parte.incendio.tipo.id}`}
        </span>
      )}
      {parte?.incendio?.fase && (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-fuchsia-200 bg-fuchsia-50 text-fuchsia-800">
          ...
          {parte.incendio.fase.nombre || `Fase #${parte.incendio.fase.id}`}
        </span>
      )}
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-5 py-6 space-y-4">
      ...
      <div className="bg-white/80 backdrop-blur-lg border border-[#4EB9FA]/20 shadow-md rounded-2xl p-6">
        <div className="flex items-start justify-between gap-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 text-sm text-gray-700 mb-3">
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  {parte?.compania?.nombre || `Compañía #${parte?.compania?.id || "-"}`}
                </span>
                {parte?.redactor && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 rounded-lg border border-gray-200">
                    <UserIcon className="h-4 w-4 text-gray-500" /> Redactor:{" "}
          ...
          <div className="w-80 space-y-4 flex-shrink-0">
            {/* Fechas */}
            <DateDisplay
              fechaCreacion={parte?.createdAt || parte?.fechaHoraDespacho}
              fechaActualizacion={parte?.updatedAt}
            />
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const redactorId =
                        ...
                        Number(JSON.parse(localStorage.getItem("bombero") || "{}")?.id) ||
                        null;
                      await cambiarEstadoIncidente(parte.id, {
                        estado: "ENVIADO",
                        idBombero: redactorId,
                      });
      ...
      <Card className="shadow-sm border border-gray-200" title="Antecedentes generales">
        <div className="flex items-stretch gap-4 text-base text-gray-900">
          ...
          <div className="flex-1 flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <i className="pi pi-building text-2xl text-gray-700" />
              <span className="font-semibold">Compañía:</span>
              <span>
                {parte?.compania?.nombre || (parte?.compania?.id ? `#${parte?.compania?.id}` : "-")}
              </span>
            </div>
            <div className="flex items-center gap-3">
              <i className="pi pi-calendar text-2xl text-gray-700" />
              <span className="font-semibold">Fecha del despacho:</span>
              <span>{fechaIncidente}</span>
            </div>
            <div className="flex items-center gap-3">
              <i className="pi pi-clock text-2xl text-gray-700" />
              <span className="font-semibold">Hora de despacho:</span>
              <span>{parte?.horaDespacho || "-"}</span>
            </div>
          </div>
          {/* Divisor vertical */}
          ...
          <div className="flex-1 flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <i className="pi pi-map-marker text-2xl text-gray-700" />
              <span className="font-semibold">Dirección:</span>
              <span>{direccion || "-"}</span>
            </div>
            <div className="flex items-center gap-3">
              <i className="pi pi-compass text-2xl text-gray-700" />
              <span className="font-semibold">Comuna:</span>
              <span>{parte?.direccion?.comuna?.nombre || "-"}</span>
            </div>
            <div className="flex items-center gap-3">
              <i className="pi pi-building text-2xl text-gray-700" />
              <span className="font-semibold">Depto:</span>
              <span>{parte?.direccion?.depto || "-"}</span>
            </div>
            <div className="flex items-center gap-3">
              <i className="pi pi-info-circle text-2xl text-gray-700" />
              <span className="font-semibold">Referencia:</span>
              <span>{parte?.direccion?.referencia || "-"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-3">
          <div className="flex items-center gap-3">
            <Avatar
              label={(parte?.bomberoACargo?.nombreCompleto || "B")[0]}
              className="bg-indigo-100 text-indigo-700"
              shape="circle"
              size="large"
            />
            <div className="text-lg font-semibold text-gray-900">
              {parte?.bomberoACargo?.nombreCompleto ||
                (parte?.bomberoACargo?.id ? `#${parte.bomberoACargo.id}` : "-")}
            </div>
            <Badge value="A cargo" severity="info" className="ml-1" />
          </div>
          <div className="flex items-center gap-3">
            <Avatar
              label={(parte?.redactor?.nombreCompleto || "R")[0]}
              className="bg-emerald-100 text-emerald-700"
              shape="circle"
              size="large"
            />
            <div className="text-lg font-semibold text-gray-900">
              {parte?.redactor?.nombreCompleto ||
                (parte?.redactor?.id ? `#${parte.redactor.id}` : "-")}
            </div>
        ...
        <div className="flex flex-col gap-2 mb-1">
          <Timeline
            ...
            content={(item) => (
              <span className="text-[14px] text-gray-900 font-semibold">{item.label}</span>
            )}
          />
        </div>
      ...
      (Array.isArray(parte.vehiculos) && parte.vehiculos.length > 0) ? (
        <Card
          ...
          subTitle={
            <span className="text-gray-600">Detalle de bienes afectados y su titularidad</span>
          }
        >
          {Array.isArray(parte.inmuebles) &&
            parte.inmuebles.length > 0 &&
            (() => {
              const inmueblesData = parte.inmuebles.map((inm, i) => ({ __id: i, ...inm }));
              const direccionBody = (row) => {
                const dir = [
                  [row?.direccion?.calle || row?.calle, row?.direccion?.numero || row?.numero]
                    .filter(Boolean)
                    .join(" "),
                  row?.depto ? `Depto ${row.depto}` : null,
                ]
                  .filter(Boolean)
                  .join(" · ");
              ...
              return (
                <div className="mb-4">
                  ...
                  <DataTable
                    value={inmueblesData}
                    dataKey="__id"
                    expandedRows={expandedInmuebles}
                    onRowToggle={(e) => setExpandedInmuebles(e.data)}
                          ...
                          value={[
                            {
                              ...
                              gravedad: row.dueno?.descripcionGravedad ?? "-",
                              esEmpresa:
                                typeof row.dueno?.esEmpresa === "boolean"
                                  ? row.dueno.esEmpresa
                                    ? "Sí"
                                    : "No"
                                  : "-",
                            ...
                            ocupantes.length
                              ? ocupantes.map((o) => ({
                                  ...
                                  vinculo:
                                    o?.vinculo?.nombre ??
                                    o?.vinculo ??
                                    (o.esChofer ? "Chofer" : "Pasajero"),
                                  esEmpresa:
                                    typeof o?.esEmpresa === "boolean"
                                      ? o.esEmpresa
                                        ? "Sí"
                                        : "No"
                                      : "-",
              ...
              return (
                <div>
                  ...
                  <DataTable
                    value={vehiculosData}
                    dataKey="__id"
                    expandedRows={expandedVehiculos}
                    onRowToggle={(e) => setExpandedVehiculos(e.data)}
                    rowExpansionTemplate={expansionVeh}
                    size="small"
                    paginator
                    rows={5}
                    className="border border-gray-200 rounded-md"
      ...
      {Array.isArray(parte.materialMayor) && parte.materialMayor.length > 0 && (
        <Card title="Material mayor" subtitle="Recursos movilizados">
          <div className="border border-gray-200 rounded-md">
            <DataTable
              value={parte.materialMayor.map((r) => ({
                unidad: r.unidad?.patente || (r.unidad?.id ? `Unidad #${r.unidad.id}` : "-"),
                conductor:
                  r.conductor?.nombreCompleto ||
                  (r.conductor?.id ? `Bombero #${r.conductor.id}` : "-"),
                jefeUnidad:
                  r.jefeUnidad?.nombreCompleto ||
                  (r.jefeUnidad?.id ? `Bombero #${r.jefeUnidad.id}` : "-"),
      ...
      {Array.isArray(parte.otrosServicios) && parte.otrosServicios.length > 0 && (
        <Card
          ...
          title={
            <div className="flex items-center gap-2">
              ...
              <span>Otros servicios de emergencia en el lugar</span>
            </div>
          }
        ...
        >
          <div className="border border-gray-200 rounded-md">
            <DataTable
              value={parte.otrosServicios.map((s) => ({
      ...
      {Array.isArray(parte.accidentados) && parte.accidentados.length > 0 && (
        <Card
          ...
          title={
            <div className="flex items-center gap-2">
              <Users className="h-4 w-4 text-gray-700" /> <span>Bomberos accidentados</span>
            </div>
          }
        ...
        >
          <div className="border border-gray-200 rounded-md">
            <DataTable
              value={parte.accidentados.map((a) => ({
      ...
      {parte.asistencia && (
        <Card
          ...
          title={
            <div className="flex items-center gap-2">
              <Users className="h-4 w-4 text-gray-700" /> <span>Asistencia a la emergencia</span>
            </div>
          }
                ...
                <div className="border border-gray-200 rounded-md">
                  <DataTable
                    value={parte.asistencia.lugar.map((bombero, index) => ({
                      id: index,
                      bombero:
                        bombero?.nombreCompleto || (bombero?.id ? `Bombero #${bombero.id}` : "-"),
                      run: bombero?.run || "-",
                    }))}
                    size="small"
      ...
      <Card
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span>Historial de Estados y Comentarios</span>
          </div>
        }
      >
        <HistorialEstados incidenteId={id} />
      </Card>
    </div>
  );
}
