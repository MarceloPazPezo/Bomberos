...
export const useProfile = () => {
  ...
  const { bombero, updateBombero } = useAuth();
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      if (response.status === 'Success') {
        ...
        if (updateBombero && bombero) {
          // Combinar los datos actualizados con los roles y permisos existentes
          const updatedBomberoData = {
            ...response.data,
            // Conservar roles y permisos del bombero actual si no vienen en la respuesta
            roles: response.data.roles || bombero.roles || [],
            permisos: response.data.permisos || bombero.permisos || []
          };
          updateBombero(updatedBomberoData);
        }
